...
class _FinancialCalendarScreenState extends State<FinancialCalendarScreen> {
  ...
  // ── shared scroll for mode-1 header / body sync ──
  final ScrollController _headerScroll = ScrollController();
  final ScrollController _bodyScroll = ScrollController();

  // Column widths (mode 1)
  static const double _cName = 110;
  static const double _cType = 34;
  static const double _cOrig = 72;
  static const double _cEff = 82;
  static const double _cPlan = 58;
  static const double _cPaid = 70;
  static const double _cDue = 70;
  ...
  void _loadMode1() {
    ...
    // filter events whose logical date falls in this month
    ...
    // pending = from students admitted this month who still owe
  ...
  void _loadMode2() {
    final students =
        CacheService.read(
          'students',
        ).where((s) => s['is_deleted'] != true).toList()..sort(
  ...
  Widget _buildMode2Filters() {
    ...
    final cnt = <String, int>{
      ...
      'PENDING': _students
          ...
          .length,
      'EXPIRED': _students
          .where((s) => (s['end_date']?.toString() ?? '').compareTo(today) < 0)
          .length,
    };
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: ['ALL', 'PAID', 'PARTIAL', 'PENDING', 'EXPIRED'].map((f) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: active ? c.withOpacity(0.12) : const Color(0xFFF1F5F9),
                borderRadius: BorderRadius.circular(8),
                border: Border.all(
  ...
  List<Map<String, dynamic>> get _filteredStudents {
    ...
    return _students.where((s) {
      final expired = (s['end_date']?.toString() ?? '').compareTo(today) < 0;
  ...
  Widget _studentCard(Map<String, dynamic> s) {
    ...
    final today = DateTime.now().toIso8601String().split('T')[0];
    final isExpired = (s['end_date']?.toString() ?? '').compareTo(today) < 0;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: expanded ? sc.withOpacity(0.4) : const Color(0xFFE2E8F0),
